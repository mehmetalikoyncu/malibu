...
module.exports = async (client, queue, track) => {
  ...
  const collector = nowplay.createMessageComponentCollector({ filter, time: 1200000 });
...
function disspace(nowQueue, nowTrack) {
  const embeded = new EmbedBuilder()
    ...
    .addFields({ name: `Şuanki Süre: \`[00:00 / ${nowTrack.formattedDuration}]\``, value:`\`\`\`🔴 | 🎶──────────────────────────────\`\`\``, inline: false })
    .setTimestamp()
