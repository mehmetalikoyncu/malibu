...
module.exports = {
    ...
    run: async (client, message, args) => {
        const embed = new EmbedBuilder()
            .setColor('#000001')
            .setAuthor({ name: `${message.guild.members.me.displayName} Yardım Komutu!`, iconURL: message.author.displayAvatarURL({ dynamic: true })})
        ...
        if(!args[0]) {
            ...
            categories.forEach(kategori => {
                ...
                try {
                    embed.addFields({ name: `❯ ${capitalise} [${dir.size}]:`, value: dir.map(c => `\`${c.config.kullanım}\``).join(" "), inline: false })
                } catch(e) {
                    console.log(e)
                }
            })
